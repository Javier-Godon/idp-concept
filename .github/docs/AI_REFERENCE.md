...
_render_stack = renderstack.RenderStack {
...
kcl_to_yaml.yaml_stream_stack(factory_seed._render_stack)
```
...
Accessory ← PostgreSQLClusterModule ← ConcretePgModule
Accessory ← MongoDBCommunityModule ← ConcreteMongoModule
Accessory ← RabbitMQClusterModule ← ConcreteRmqModule
Accessory ← RedisModule ← ConcreteRedisModule
Accessory ← KeycloakModule ← ConcreteKcModule
Accessory ← OpenSearchClusterModule ← ConcreteOsModule
Accessory ← VaultStaticSecretModule ← ConcreteVaultModule
Accessory ← QuestDBModule ← ConcreteQdbModule
Stack ← ConcreteStack (adds namespaces, components, accessories)
RenderStack ← wraps Stack for multi-format rendering
...
| `build_network_policy` | `NetworkPolicySpec` | `NetworkPolicy` |
| `build_pdb` | `PDBSpec` | `PodDisruptionBudget` |
| `build_external_secret` | `ExternalSecret` | `ExternalSecret CRD` |
| `build_thirdparty_helm` | `ThirdPartyHelmSpec` | `HelmRelease manifest` |

### Operator Template Build Functions

| Builder | Input Schema | CRD API |
|---|---|---|
| `build_cnpg_cluster` | `CNPGClusterSpec` | `postgresql.cnpg.io/v1 Cluster` |
| `build_pooler` | `PoolerSpec` | `postgresql.cnpg.io/v1 Pooler` |
| `build_scheduled_backup` | `ScheduledBackupSpec` | `postgresql.cnpg.io/v1 ScheduledBackup` |
| `build_mongodb_community` | `MongoDBCommunitySpec` | `mongodbcommunity.mongodb.com/v1 MongoDBCommunity` |
| `build_rabbitmq_cluster` | `RabbitMQClusterSpec` | `rabbitmq.com/v1beta1 RabbitmqCluster` |
| `build_redis` | `RedisSpec` | `redis.redis.opstreelabs.in/v1beta2 Redis/RedisCluster` |
| `build_keycloak` | `KeycloakSpec` | `k8s.keycloak.org/v2alpha1 Keycloak` |
| `build_opensearch_cluster` | `OpenSearchClusterSpec` | `opensearch.org/v1 OpenSearchCluster` |
| `build_vault_connection` | `VaultConnectionSpec` | `secrets.hashicorp.com/v1beta1 VaultConnection` |
| `build_vault_auth` | `VaultAuthSpec` | `secrets.hashicorp.com/v1beta1 VaultAuth` |
| `build_vault_static_secret` | `VaultStaticSecretSpec` | `secrets.hashicorp.com/v1beta1 VaultStaticSecret` |
| `build_questdb_release` | `QuestDBSpec` | ThirdPartyHelmSpec (Helm chart) |
| `build_minio_tenant` | `MinIOTenantSpec` | `minio.min.io/v2 Tenant` (operator CRD) |
| `build_minio_helm` | `MinIOHelmSpec` | ThirdPartyHelmSpec (Bitnami Helm chart) |
...
└── pre_releases/
    ...
    └── manifests/dev/factory/
        ├── factory_seed.k         (exports _stack, _project_name, _git_repo_url, _manifest_path)
        └── render.k               (generic multi-format renderer, identical everywhere)
```
