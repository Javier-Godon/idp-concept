...
## Rules
1. **Pre-releases** go under `pre_releases/manifests/<site>/factory/`
2. **Releases** go under `releases/<format>/<site>/<version>/factory/`
3. Format is one of: `helmfile`, `kusion`, `yaml`
4. Create a `factory/` directory with:
   - `factory_seed.k` — Imports project, tenant, site, profile, merges configs, creates stack + RenderStack
   - Format-specific builders
5. For yaml: need `yaml_builder.k`
...
- Output format (yaml, helmfile, kusion)
