...
Generated Crossplane output must keep sequencer rules aligned with the actual resource names emitted in the `Resources` input. Namespace dependencies are rendered as `ns-<namespace>` Crossplane resource names, while component/accessory dependencies use `comp-<name>-.*` and `acc-<name>-.*` patterns to cover every manifest generated by the module.
