...
Generated Crossplane output must keep sequencer rules aligned with the actual resource names emitted in the `Resources` input. Namespace, component, and accessory dependencies now resolve to concrete wrapped `Object` resource names (for example `ns-apps`, `acc-postgres-statefulset-postgres`, `comp-api-deployment-api`) so sequencing follows the exact rendered graph. Regex patterns are only used as a fallback when a declared dependency is not part of the rendered stack.
