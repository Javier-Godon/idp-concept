...
Acceptance tests live outside the default `./scripts/verify.sh` path because they require Docker, kind, image pulls, and more time.
...
| `dataprepper` | standalone Data Prepper workload | `./scripts/acceptance_kind.sh --case dataprepper` |
| `search` | OpenSearch Dashboards, Elasticsearch OSS, Kibana OSS, and Logstash OSS server-side dry-run cases | `./scripts/acceptance_kind.sh --case search` |

`basic`, `webapp`, `database`, and `dataprepper` apply resources and wait for
rollout. Search-family cases render and pass server-side dry-run, but skip apply
because they pull heavier images or expect backing services.
...
See `docs/ACCEPTANCE_TESTING.md` for details and future grouped scenarios such as `postgres-keycloak`, `dataprepper-opensearch`, and `elastic-eck-v9`.
